...
export class VueWorker {
   ...
   format(uri: string, range: ls.Range, options: ls.FormattingOptions): Thenable<ls.TextEdit[]> {
      ...
      for (const mode of this.languageModes.getAllModes()) {
         if (mode.getId() === 'javascript') jsMode = mode;
      }
      const _range = this.languageModes.documentRegions.get(document).getLanguageRanges({start, end}).find(r => r.languageId === 'javascript');
      const text = jsMode.format(document, _range, options, null)
      try {
   ...
   doAutoClose(uri: string, position: Position): Thenable<any> {
      let document = this._getTextDocument(uri);
      let mode = this.languageModes.getModeAtPosition(document, position);
      if (mode && mode.doAutoClose) {
         return Promise.as(mode.doAutoClose(document, position));
      }
   }
